...
const cvFormatter = (arr) => {
    // write your code here
    let formattedCvsArr = [];
    let formattedCvsOject = {};

    for (let i = 0; i < arr.length; i++) {
        if (arr[i].yearsOfExperience > 1) {
            if (arr[i].firstName == null) {
                formattedCvsOject = {};
                formattedCvsOject.fullName = arr[i].lastName;
                formattedCvsOject.tech = arr[i].tech;
                formattedCvsArr.push(formattedCvsOject);
            }
            if (arr[i].lastName == null) {
                formattedCvsOject = {};
                formattedCvsOject.fullName = arr[i].firstName;
                formattedCvsOject.tech = arr[i].tech;
                formattedCvsArr.push(formattedCvsOject);
            }
            else {
                formattedCvsOject = {};
                formattedCvsOject.fullName = arr[i].firstName + " " + arr[i].lastName;
                formattedCvsOject.tech = arr[i].tech;
                formattedCvsArr.push(formattedCvsOject);
            }
    ...
    console.log(formattedCvsArr);
    return formattedCvsArr;
};
// -------------------------------------------------------------------------------------------------------
...
const applicationsStatics = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].yearsOfExperience <= 1 || arr[i].firstName == null ||
            arr[i].firstName === " " || arr[i].lastName == null ||
            arr[i].lastName === " ") {
...
const classesAvg = (data) => {
    ...
    let classScoresNumber = 0;

    for (let j = 0; j < data.grades.length; j++) {
        for (let e = 0; e < data.grades[j].classes.length; e++) {
            // console.log("in grads " +j+" classe "+ e);
            for (let x = 0; x < data.grades[j].classes[e].classScores.length; x++) {
                //  console.log("in classe " +e+" score "+ x);
                classScoresTotal += data.grades[j].classes[e].classScores[x];
                classScoresNumber++;
            }
            // console.log(classScoresTotal);
            // console.log(classScoresNumber);
            data.grades[j].classes[e].avg = Math.floor(classScoresTotal / classScoresNumber);
            classScoresTotal = 0;
            classScoresNumber = 0;
        }
    }
